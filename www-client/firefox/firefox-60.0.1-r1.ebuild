...
src_prepare() {
	...
	use wayland && eapply "${FILESDIR}"/remove-gtk2.patch
	use wayland && eapply "${FILESDIR}"/disable-x11.patch
	use wayland && eapply "${FILESDIR}"/audioipc.patch
...
src_configure() {
	...
	use wayland && mozconfig_annotate 'Enable EGL as GL provider' --with-gl-provider=EGL
	use wayland && sed -e "s/'gtk2', //" -i "${S}"/widget/gtk/mozgtk/moz.build
	#use wayland && rm "${S}"/widget/gtk/mozgtk/gtk2/moz.build
...
src_compile() {
	# set MOZ_X11=0 if use wayland
	use wayland && sed -e 's/MOZ_X11=1/MOZ_X11=0/' -i "${S}"/ff/old-configure.vars
